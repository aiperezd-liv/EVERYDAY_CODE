...
------------------------------------------------------------------------------------------------------------------------
-- MÓDULO 1: SOLICITUDES Y DEFINICIÓN DEL ESCENARIO HIPOTÉTICO
------------------------------------------------------------------------------------------------------------------------
WITH LINEAS AS (
  SELECT
      BR_ING_TOT,              -- Ingreso total del cliente
      CTA_CVE,                 -- Clave única de la cuenta
      BR_IMP_LIM_CRED,         -- Límite de crédito asignado real (Observado)
      ...
      -- REGLA DE NEGOCIO / ESCENARIO HIPOTÉTICO:
      -- Si el ingreso es suficiente (>=4000) y su LC real era menor a 4000, se simula un aumento a 4000.
      CASE 
        WHEN BR_ING_TOT >= 4000 AND BR_IMP_LIM_CRED < 4000 THEN 4000 
        ELSE BR_IMP_LIM_CRED
      END AS NEW_LC
  FROM `crp-pro-dwh-semanticagold.EIL_DP_VMASTER.VFAC_NEGFIN_SOLICITUDES`
  WHERE DT_FCH_SOL BETWEEN '2025-03-01' AND '2025-08-31'  -- <<< RANGO TEMPORAL DEL ANÁLISIS
...
CALC_SDO_PROM AS (
  SELECT 
    ...
    -- Tasas de morosidad grupales (Cuentas con mora / Total cuentas activas en el MOB correspondiente)
    ...
    -- Porcentajes promedio de utilización de línea de crédito (Saldo Actual / Límite de Crédito)
    AVG(SAFE_DIVIDE(b.SDO_ENTRY_2M, b.CTA_IMP_LIM_CRD)) AS AVG_UTIL_ENTRY_2M,
    AVG(SAFE_DIVIDE(b.SDO_TOT_2M, b.CTA_IMP_LIM_CRD))   AS AVG_UTIL_TOT_2M,
    AVG(SAFE_DIVIDE(b.SDO_TOT_3M, b.CTA_IMP_LIM_CRD))   AS AVG_UTIL_TOT_3M,
    AVG(SAFE_DIVIDE(b.SDO_30_3M, b.CTA_IMP_LIM_CRD))    AS AVG_UTIL_30_3M,
    AVG(SAFE_DIVIDE(b.SDO_TOT_6M, b.CTA_IMP_LIM_CRD))   AS AVG_UTIL_TOT_6M,
    AVG(SAFE_DIVIDE(b.SDO_30_6M, b.CTA_IMP_LIM_CRD))    AS AVG_UTIL_30_6M,
    AVG(SAFE_DIVIDE(b.SDO_TOT_9M, b.CTA_IMP_LIM_CRD))   AS AVG_UTIL_TOT_9M,
    AVG(SAFE_DIVIDE(b.SDO_90_9M, b.CTA_IMP_LIM_CRD))    AS AVG_UTIL_90_9M,
    AVG(SAFE_DIVIDE(b.SDO_TOT_12M, b.CTA_IMP_LIM_CRD))  AS AVG_UTIL_TOT_12M,
    AVG(SAFE_DIVIDE(b.SDO_90_12M, b.CTA_IMP_LIM_CRD))   AS AVG_UTIL_90_12M
  FROM LINEAS a
...
NUEVAS_MORAS AS (
  SELECT
      *,
      DATE_TRUNC(CTA_FCH_ALTA, MONTH) AS COSECHA, -- Mes de apertura de la cuenta (Cosecha)
      
      -- Recálculo de saldos estimados teóricos basados en NEW_LC
      CASE WHEN CTA_IMP_LIM_CRD > 0 THEN SAFE_DIVIDE(SDO_TOT_2M, CTA_IMP_LIM_CRD) * NEW_LC ELSE AVG_UTIL_TOT_2M * NEW_LC END AS SDO_TOT_2M_NEW,
      CASE WHEN CTA_IMP_LIM_CRD > 0 THEN SAFE_DIVIDE(SDO_ENTRY_2M, CTA_IMP_LIM_CRD) * NEW_LC ELSE AVG_UTIL_ENTRY_2M * NEW_LC * BR_CTA_ENTRY_2M END AS SDO_ENTRY_2M_NEW,
      
      CASE WHEN CTA_IMP_LIM_CRD > 0 THEN SAFE_DIVIDE(SDO_TOT_3M, CTA_IMP_LIM_CRD) * NEW_LC ELSE AVG_UTIL_TOT_3M * NEW_LC END AS SDO_TOT_3M_NEW,
      CASE WHEN CTA_IMP_LIM_CRD > 0 THEN SAFE_DIVIDE(SDO_30_3M, CTA_IMP_LIM_CRD) * NEW_LC ELSE AVG_UTIL_30_3M * NEW_LC * BR_CTA_30_3M END AS SDO_30_3M_NEW,
      
      CASE WHEN CTA_IMP_LIM_CRD > 0 THEN SAFE_DIVIDE(SDO_TOT_6M, CTA_IMP_LIM_CRD) * NEW_LC ELSE AVG_UTIL_TOT_6M * NEW_LC END AS SDO_TOT_6M_NEW,
      CASE WHEN CTA_IMP_LIM_CRD > 0 THEN SAFE_DIVIDE(SDO_30_6M, CTA_IMP_LIM_CRD) * NEW_LC ELSE AVG_UTIL_30_6M * NEW_LC * BR_CTA_30_6M END AS SDO_30_6M_NEW,
      
      CASE WHEN CTA_IMP_LIM_CRD > 0 THEN SAFE_DIVIDE(SDO_TOT_9M, CTA_IMP_LIM_CRD) * NEW_LC ELSE AVG_UTIL_TOT_9M * NEW_LC END AS SDO_TOT_9M_NEW,
      CASE WHEN CTA_IMP_LIM_CRD > 0 THEN SAFE_DIVIDE(SDO_90_9M, CTA_IMP_LIM_CRD) * NEW_LC ELSE AVG_UTIL_90_9M * NEW_LC * BR_CTA_90_9M END AS SDO_90_9M_NEW
  FROM CONSOLIDADO
),

------------------------------------------------------------------------------------------------------------------------
-- MÓDULO 7: AGREGACIÓN POR VARIABLE DE APERTURA / SUBMUESTRA TARGET
------------------------------------------------------------------------------------------------------------------------
AGRUPADO AS (
  SELECT 
    BR_HIT_DES,
  ...
  FROM NUEVAS_MORAS
  WHERE BR_HIT_DES IN ('HIT', 'THIN FILE', 'NOHIT')  -- Filtro de limpieza de segmentos válidos en Buró
  GROUP BY 1
)

------------------------------------------------------------------------------------------------------------------------
-- OUTPUT FINAL: CÁLCULO DE PAREJAS DE CONTROL, DIFERENCIAS ABSOLUTAS Y PORCENTUALES
------------------------------------------------------------------------------------------------------------------------
SELECT 
  BR_HIT_DES,
  Solicitudes,
